...
module.exports = {
    ...
    async execute(client, modal) {
        ...
        const response = await fetch('https://yuri.bots.lostluma.dev/phish/report', { 
            method: 'POST', 
            body: JSON.stringify({ url:domain, reason:`Reported by \`${modal.user.username}#${modal.user.discriminator}\` via the Anti.fish API server.\nReport reason: ${reason}\nAdditional details: ${extra}` }), 
            headers: {
        ...
        if (data.accepted) {
            ...
            await modal.reply({ content:`Successfully reported **${domain}** to fishfish.gg!`, ephemeral:true })
